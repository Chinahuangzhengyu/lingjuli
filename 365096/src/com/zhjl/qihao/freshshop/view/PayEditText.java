 ...
 * 支付密码输入框
 */
public class PayEditText extends android.support.v7.widget.AppCompatEditText {
    ...
    //设置间距的
    private RectF rectFPadding = new RectF();
    ...
    private void getAtt(AttributeSet attrs) {
        ...
        radius = typedArray.getDimensionPixelOffset(R.styleable.PayPsdInputView_psdRadius, radius);
    ...
    private void initPaint() {
        ...
        paddingPaint = getPaint(2, Paint.Style.FILL, Color.WHITE);
    }
...
//        canvas.drawRoundRect(rectF, rectAngle, rectAngle, borderPaint);
        for (int i = 0; i < maxCount; i++) {
            rectFPadding.set((i * 1) * divideLineWStartX + Utils.dip2px(mContext, 8), 0, (i + 1) * divideLineWStartX, height);
            canvas.drawRoundRect(rectFPadding, rectAngle, rectAngle, borderPaint);
...
//                    divideLinePaint);
    ...
    private void drawPsdCircle(Canvas canvas) {
        for (int i = 0; i < textLength; i++) {
            canvas.drawCircle(startX + i * 2 * startX + Utils.dip2px(mContext, 4),
                    startY,
                    radius,
                    circlePaint);
        }
